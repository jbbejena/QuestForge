...
MODEL = os.getenv("OPENAI_MODEL", "gpt-5")  # set to gpt-5 later
...
def _repo_snapshot():
    ...
    for p in _list_candidate_files():
        try:
            ...
            continue
        if len(b) > MAX_FILE_BYTES: b = b[:MAX_FILE_BYTES]
        if total + len(b) > MAX_TOTAL_BYTES: break
        total += len(b)
...
def try_git_commit(message: str) -> int:
    ...
    if not (PROJECT_ROOT/".git").exists():
        run(["git","init"]); run(["git","add","."]); return run(["git","commit","-m","Initial commit"])
    run(["git","add","."]); return run(["git","commit","-m",message])